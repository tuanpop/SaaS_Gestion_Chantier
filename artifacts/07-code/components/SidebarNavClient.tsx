...
export function SidebarNavClient({ inSheet = false }: SidebarNavClientProps = {}) {
  ...
  const navItems = [
    ...
    // Sprint 7 — Briefings lundi (US-061 reachability UI — V-7-14 BINDING)
    {
      ...
      label: 'Briefings',
      testId: 'nav-link-briefings',
      // RBAC: visible admin only (ouvrier/conducteur pas d'accès direct)
      icon: (
        <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          {/* Sun icon — spec US-059 BINDING (distinct de FileText vert du rapport hebdo) */}
