...
export const dateOrderRefinement = {
  check: (data: { date_debut: string; date_fin_prevue: string }) =>
    data.date_fin_prevue >= data.date_debut,
  message: 'date_fin_prevue doit être >= date_debut',
  path: ['date_fin_prevue'] as const,
}

export const CreateChantierBaseSchema = z.object({
...
export const CreateChantierSchema = CreateChantierBaseSchema.refine(
